...
// Ao rolar a página, recolhe o menu inteiro (fundo + itens) em vez de
// deixar itens presos na tela, e atualiza o estilo do header.
const onScroll = () => {
  header.classList.toggle('scrolled', scrollY > 40);
  closeNav();
};
// Adiado para depois da primeira pintura: chamado direto aqui, o toggle de
// classe logo após a leitura de scrollY forçava um reflow síncrono na carga.
...
const createStars = (rating, size = 18) => {
...
const createGoogleIcon = () => {
  ...
  svg.setAttribute('aria-hidden', 'true');
  const paths = [
    { fill: '#EA4335', d: 'M24 9.5c3.5 0 6.6 1.2 9.1 3.6l6.8-6.8C35.8 2.4 30.2 0 24 0 14.6 0 6.5 5.4 2.6 13.2l7.9 6.2C12.4 13.4 17.7 9.5 24 9.5z' },
    { fill: '#4285F4', d: 'M46.1 24.5c0-1.6-.1-3.1-.4-4.5H24v9.1h12.4c-.5 2.9-2.2 5.4-4.6 7.1l7.5 5.8c4.4-4.1 6.8-10.1 6.8-17.5z' },
    { fill: '#FBBC05', d: 'M10.5 28.6c-.5-1.5-.8-3-.8-4.6s.3-3.1.8-4.6l-7.9-6.2C.9 16.5 0 20.1 0 24s.9 7.5 2.6 10.8l7.9-6.2z' },
    { fill: '#34A853', d: 'M24 48c6.2 0 11.4-2 15.2-5.5l-7.5-5.8c-2.1 1.4-4.7 2.2-7.7 2.2-6.3 0-11.6-3.9-13.5-9.3l-7.9 6.2C6.5 42.6 14.6 48 24 48z' },
  ];
...
const renderGoogleReviews = async () => {
  ...
  try {
    ...
    gStars.replaceChildren(createStars(Math.round(data.rating), 22));
    ...
    for (const [i, review] of data.reviews.entries()) {
      ...
      stars.replaceChildren(createStars(review.rating));
    ...
    // Sem JSON acessível: link direto para o Google como fallback
    const p = document.createElement('p');
    p.className = 'g-noscript';
    p.textContent = 'Veja nossas avaliações na ';
    const a = document.createElement('a');
    a.href = 'https://search.google.com/local/reviews?placeid=ChIJKY4kGO-juwARScvIFzKcThQ';
    a.target = '_blank';
    a.rel = 'noopener noreferrer';
    a.textContent = 'ficha do Google';
    p.append(a, '.');
    grid.append(p);
  }
};
